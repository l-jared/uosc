...
function render_timeline(this)
	...
	if text_opacity > 0 then
		...
		if state.elapsed_seconds then
			...
			ass:new_event()
			ass:append('{\\blur0\\bord0\\shad0\\1c&H'..options.color_background..'\\fn'..config.font..'\\fs'..this.font_size..bold_tag..'\\clip('..foreground_coordinates..')')
			...
			ass:new_event()
			ass:append('{\\blur0\\bord0\\shad1\\1c&H'..options.color_foreground..'\\4c&H'..options.color_background..'\\fn'..config.font..'\\fs'..this.font_size..bold_tag..'\\iclip('..foreground_coordinates..')')
			ass:append(ass_opacity(math.min(options.timeline_opacity + 0.1, 1), text_opacity))
			ass:pos(elapsed_x, elapsed_y)
			ass:an(4)
			ass:append(state.elapsed_time)
	...
	if (this.proximity_raw == 0 or this.pressed) and not (elements.speed and elements.speed.dragging) then
		...
		ass:new_event()
		ass:append('{\\blur0\\bord'..options.text_border_size..'\\shad0\\1c&H'..options.color_background_text..'\\3c&H'..options.color_background..'\\fn'..config.font..'\\fs'..(this.font_size*1.1)..bold_tag..'')
		ass:append(ass_opacity(math.min(options.timeline_opacity + 0.1, 1)))
		ass:pos(math.min(math.max(cursor.x, box_half_width_guesstimate), display.width - box_half_width_guesstimate), fay - 6)
		ass:an(2)
...
function render_speed(this)
	...
	for i = -from_to_index, from_to_index do
		...
		ass:new_event()
		ass:append('{\\blur0\\bord'..options.text_border_size..'\\shad0\\1c&H'..options.color_background_text..'\\3c&H'..options.color_foreground_text..'}')
	...
	ass:new_event()
	ass:append('{\\blur0\\bord'..options.text_border_size..'\\shad0\\1c&H'..options.color_background_text..'\\3c&H'..options.color_foreground_text..'\\fn'..config.font..'\\fs'..this.font_size..bold_tag..'}')
	ass:append(ass_opacity(options.speed_opacity, opacity))
	ass:pos(half_x, ay)
	ass:an(8)
